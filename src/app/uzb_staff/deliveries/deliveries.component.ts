...
export class DeliveriesComponent implements OnInit {
  ...
  chekId; chekName; chekphone;chekRegion;chekTuman; chekDelType;chekIzoh;
  ...
  constructor(private datePipe: DatePipe,private changeDetectorRef: ChangeDetectorRef, private http: Http, private httpClient: HttpClient,private router: Router,public authService: AuthService) {

    this.totalLeftCountItems = 0;

    this.http.get(GlobalVars.baseUrl+'/delivery_type/list', this.options)
    .subscribe(response => {
  ...
  ngOnInit() {
    ...
    this.dataTable = {
      headerRow: ['Owner ID', 'Chinese name', 'Tracking Number', 'Quantity', 'Type', 'Status', 'Actions'],
  ...
  getListOfNewDeliveries(){
    ...
    return  this.http.get(GlobalVars.baseUrl+'/delivery/list?status=in_progress&size=500' + "&deliveryType=" +this.orderFilterStatus , this.options)
      .subscribe(response => {
        ...
        for (let index = 0; index < this.allData.length; index++) {
          ...
          this.deliveryTypeText[index] = this.getDeliveriesType(element1.deliveryType);  
        }
  ...
  getListOfSentDeliveries(){
    ...
    //let filterLink = '&status='+this.orderFilterStatus+"&orderType="+this.orderFilterType+"&ownerID="+this.orderFilterOwnId+"&consignment="+this.orderFilterParty;
    return  this.http.get(GlobalVars.baseUrl+'/delivery/list?status=sent&size=500' + "&deliveryType=" +this.orderFilterStatus , this.options)
      .subscribe(response => {
  ...
  getListOfDeliveriesWithFilter(status){
    ...
    return  this.http.get(GlobalVars.baseUrl+'/delivery/list?status=in_progress&deliveryType='+ this.orderFilterStatus, this.options)
      .subscribe(response => {
        this.allData = response.json().deliviries;
    ...
    swal.fire({
      ...
      didOpen: () => {
        ...
        for (var i = 0; i < this.deliveryTypes.length; i++) {
            ...
            this.deliveryTypes[i].nameUz, '</option>');
        } 
  ...
  yuborishniBelgilash(){
    ...
    swal.fire({
      ...
      preConfirm: (valueB) => {
            ...
            this.http.post(GlobalVars.baseUrl + '/orders/client_sent?tracking_number=' + valueB ,"", this.options)
            .subscribe(response => {
  ...
  printChek1(delID,type,aownerID, aownerName, aadditionalPhoneNumber, aregion, atown){   
    ...
    this.http.post(GlobalVars.baseUrl + '/delivery/changeStatus?deliveryID='+ delID + '&status=sent', "",this.options)
    .subscribe(response => {
  ...
  printChek2(delID,type,aownerID, aownerName, aadditionalPhoneNumber, aregion, atown){   
  ...
  printChek3(type,aownerID, aownerName, aadditionalPhoneNumber, aregion, atown){   
  ...
  deleteNewDelivey(deliDI){
    ...
    swal.fire({
      ...
      if (result.value) {
        ...
        this.http.post(GlobalVars.baseUrl + '/delivery/delete?deliveryID=' + deliDI, "",this.options)
                .subscribe(response => {
