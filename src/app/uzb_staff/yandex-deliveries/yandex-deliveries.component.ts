...
@Component({
  selector: "app-yandex-deliveries",
  templateUrl: "./yandex-deliveries.component.html",
...
export class YandexDeliveriesComponent implements OnInit {
  ...
  // Fixed delivery type - Yandex only
  readonly deliveryType: string = "Yandex";
  ...
  ngOnInit() {
    // Load today's Yandex deliveries on init
    this.applyFilters();
  }
        ...
        (response) => {
          ...
          if (result.status === "success") {
            swal.fire({
              icon: "success",
              title: "Muvaffaqiyat!",
              text: `Yetkazish Yandex kuryer xizmatiga topshirildi`,
              customClass: {
                confirmButton: "btn btn-success",
              },
              buttonsStyling: false,
