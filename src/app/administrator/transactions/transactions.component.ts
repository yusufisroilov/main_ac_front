...
export class TransactionsComponent implements OnInit {
  ...
  constructor(private datePipe: DatePipe,private http: Http, private httpClient: HttpClient,private router: Router,public authService: AuthService) { 
...
getListOfTransactionsWithDate(){
  ...
  return  this.http.get(GlobalVars.baseUrl+'/transactions/list?'+ 'from_date='+ this.danValue +'&to_date=' + this.gachaValue, this.options)
    .subscribe(response => {
...
editTransaction(finID) {
  ...
  swal.fire({
    ...
    preConfirm: (result) => {
      ...
      let readyDate = split[2] + '/' + split[1] + '/' + split[0]
    
 

      this.http.post(GlobalVars.baseUrl + '/transactions/edit?transaction_id='+ finID+'&date=' + readyDate , "", this.options)
        .subscribe(response => {
  ...
  getListOfTransactions(){
    ...
    return  this.http.get(GlobalVars.baseUrl+'/transactions/list?'+ this.currentPage+'&size=200', this.options)
      .subscribe(response => {
        this.allTransactions = response.json().transactions;
        this.currentPage = response.json().currentPage;
        this.totalPages = response.json().totalPages;
  ...
  getListOfTransactionsWithFilter(clientId, partiya,){
   ...
   let filterLink = '&ownerID='+clientId+"&consignment="+partiya;
    return  this.http.get(GlobalVars.baseUrl+'/transactions/list?page'+ this.currentPage+'&size=100'+filterLink, this.options)
      .subscribe(response => {
        this.allTransactions = response.json().transactions;
       this.currentPage = response.json().currentPage;
        this.totalPages = response.json().totalPages;
